...
inline std::string getExtension(const std::string& filePath) {
    auto p = std::experimental::filesystem::path(filePath);
...
inline static int fileWrite(const std::string& name, const char* bulk, size_t len, bool append = false, bool exclusive = false)
{
    FILE* f = NULL;
    if(append)
